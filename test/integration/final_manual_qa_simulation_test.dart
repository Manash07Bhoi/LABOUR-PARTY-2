...
void main() {
  ...
  test('Data Integrity Audit - No Hidden Duplication', () async {
    ...
    final work1 = Work(id: 'work_24_May_2026_Morning', date: '24 May 2026', session: 'Morning', workType: 'Sand', place: '', createdAt: DateTime.now(), updatedAt: DateTime.now());
    final trip1 = Trip(id: 't1', workId: work1.id, tripNumber: 1, tractor: 'T1', driverName: 'D1', createdAt: DateTime.now());
    final l1 = Labour(id: 'l1', name: 'Rahul', createdAt: DateTime.now());
    final tl1 = TripLabour(id: 'tl1', tripId: 't1', labourId: 'l1', isPresent: true);

    bloc.add(SaveFullWorkTripEvent(work: work1, trip: trip1, tripLabours: [tl1], labours: [l1]));
    ...
    final trip1Edit = Trip(id: 't1', workId: work1.id, tripNumber: 1, tractor: 'T2_Edited', driverName: 'D2_Edited', createdAt: trip1.createdAt);
    ...
    bloc.add(SaveFullWorkTripEvent(work: work1, trip: trip1Edit, tripLabours: [tl2], labours: [l2]));
    await Future.delayed(const Duration(milliseconds: 100));

    // Verify lengths did not duplicate orphaned records
    expect(repo.works.length, 1);
    expect(repo.trips.length, 1);
    expect(repo.labours.length, 2); // Labour entities persist globally historically
    expect(repo.tripLabours.length, 1); // TripLabour replaced l1 with l2 cleanly
    expect(repo.tripLabours.first.labourId, 'l2');
    
    // Scenario C: Multi-Day Isolation
    final work2 = Work(id: 'work_25_May_2026_Morning', date: '25 May 2026', session: 'Morning', workType: 'Sand', place: '', createdAt: DateTime.now(), updatedAt: DateTime.now());
    final trip2 = Trip(id: 't2', workId: work2.id, tripNumber: 1, tractor: 'T1', driverName: 'D1', createdAt: DateTime.now());
    final tl3 = TripLabour(id: 'tl3', tripId: 't2', labourId: 'l2', isPresent: true);

    bloc.add(SaveFullWorkTripEvent(work: work2, trip: trip2, tripLabours: [tl3], labours: [l2]));
    await Future.delayed(const Duration(milliseconds: 100));
